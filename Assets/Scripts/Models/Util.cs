...
public class Util : SA_Singleton<Util>
{
	...
	#region UTIL_SONTH
	public static IList DeserializeJsonArrayToList(string jsonString){
		IList tweets = ANMiniJSON.Json.Deserialize(jsonString) as IList;
		return tweets;
	}

	public static IDictionary DeserializeJsonToDict(string jsonString){
		IDictionary tweets = ANMiniJSON.Json.Deserialize(jsonString) as IDictionary;
		return tweets;
	}

	public delegate void LoadPictureCallback(Texture2D texture, string tableID);
	public static LoadPictureCallback loadPictureCB = null;

	public void LoadPictureFromURL(string url, string tableID){
		StartCoroutine (LoadPicture(url, tableID));
	}

	IEnumerator LoadPicture(string url, string tableID){
		WWW imgLink = new WWW (url);
		yield return imgLink;
		loadPictureCB (imgLink.texture, tableID);
