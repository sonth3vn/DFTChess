...
public class AddressBookController : SA_Singleton<AddressBookController> {
	...
	private List<AndroidContactInfo> contacts = new List<AndroidContactInfo>();
	...
	private void OnContactsLoaded(string data)  {
		...
		_isLoaded = true;
		Debug.Log ("OnContactsLoaded, total:" + contacts.Count);
		dispatch(ON_CONTACTS_LOADED, contacts);
	}
