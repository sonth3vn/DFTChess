...
public class ISDSettingsEditor : Editor
{
	...
	private void Frameworks() {
		...
		if(ISDSettings.Instance.IsfwSettingOpen) {
			...
			newFreamwork = GUILayout.TextField(newFreamwork, GUILayout.Width(200));
			EditorGUILayout.EndHorizontal();
	...
	private void LinkerFlags() {
		...
		if(ISDSettings.Instance.IslinkerSettingOpne) {
			...
			foreach(string flasg in ISDSettings.Instance.linkFlags) {
				...
				EditorGUILayout.BeginHorizontal();
				EditorGUILayout.Foldout(false, flasg);
			...
			NewLinkerFlag = GUILayout.TextField(NewLinkerFlag, GUILayout.Width(200));
			EditorGUILayout.EndHorizontal();
	...
	private void CompilerFlags() {
		...
		if(ISDSettings.Instance.IscompilerSettingsOpen) {
			...
			EditorGUILayout.LabelField("Add New Flag");
			NewCompilerFlag = GUILayout.TextField(NewCompilerFlag, GUILayout.Width(200));
			EditorGUILayout.EndHorizontal();
	...
	/*
	 * 
	 * ISDSettings settings = target as ISDSettings;
		EditorGUILayout.BeginVertical ();

		EditorGUILayout.HelpBox ("Enter FRAMEWORKS separated by a space", MessageType.Info);

		EditorGUILayout.BeginHorizontal ();
		EditorGUILayout.LabelField ("Frameworks:", new GUILayoutOption[] { GUILayout.Width (100) });
		EditorGUI.BeginChangeCheck ();
		settings.frameworks = EditorGUILayout.TextField (settings.frameworks);
		...
		EditorGUILayout.HelpBox ("Enter CFLAGS separated by a space", MessageType.Info);

		EditorGUILayout.BeginHorizontal ();
		EditorGUILayout.LabelField ("Compile Flags:", new GUILayoutOption[] { GUILayout.Width (100) });
		EditorGUI.BeginChangeCheck ();
		settings.compileFlags = EditorGUILayout.TextField (settings.compileFlags);
		if(EditorGUI.EndChangeCheck ())
		{
			DirtyEditor ();
		}
		...
		EditorGUILayout.HelpBox ("Enter LDFLAGS separated by a space", MessageType.Info);

		EditorGUILayout.BeginHorizontal ();
		EditorGUILayout.LabelField ("Link Flags:", new GUILayoutOption[] { GUILayout.Width (100) });
		EditorGUI.BeginChangeCheck ();
		settings.linkFlags = EditorGUILayout.TextField (settings.linkFlags);
		if(EditorGUI.EndChangeCheck ())
		{
			DirtyEditor ();
		}
