...
//																					  : Color.white;
					//edit by sonth
					_panelItems[i].img.color = new Color(0, 0, 0, 0);
					_panelItems[i].txt.fontSize = 20;
					_panelItems[i].txt.alignment = TextAnchor.MiddleLeft;
					...
					{
						OnItemClicked(ii);
						if (item.OnSelect != null) item.OnSelect(item.ID, item.Value);
					});
				}
		...
		//sonth: hack clone of OnItemClicked
		public void CloneOnItemClicked(int indx)
		{
			_isPanelActive = true;
			OnItemClicked (indx);
			DropDownListItem item = Items[indx];
			if (item.OnSelect != null) item.OnSelect(item.ID, item.Value);
		}
		...
		private void UpdateSelected()
		{
			...
			{
				if (_selectedIndex == i){
					_panelItems[i].btnImg.color = Color.grey;
				}
				else{
